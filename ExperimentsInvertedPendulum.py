...
class ExperimentsInvertedPendulum:
    ...
    def _run_episode(domain_agents, num_steps=100, r_maxs=None, optimize_meta=True):
        ...
        for d, agent_group in enumerate(domain_agents):
            for k, agent in enumerate(agent_group):
                ...
                thread = Thread(target=ExperimentsInvertedPendulum.thread_train, args=(d, k, domain_agents, num_steps, traj_by_domain, max_r, optimize_meta))
    ...
    def experiment_train_meta(save_directory, meta_alpha, meta_beta, xml_models): 
        ...
        for k in range(meta_train_episodes):
            ...
            for ep in range(episodes):
                ...
                trajectories_by_domain = ExperimentsInvertedPendulum._run_episode(domain_agents=agents, num_steps=steps, r_maxs=None)
    ...
    def experiment_meta_vs_random(meta_actor, meta_critic, save_directory, xml_models, episodes, steps):    
        ...
        for k in range(1):
            ...
            for ep in range(episodes):
                null_actions = {}
                trajectories_by_domain = ExperimentsInvertedPendulum._run_episode(domain_agents=agents, num_steps=steps, optimize_meta=False)
